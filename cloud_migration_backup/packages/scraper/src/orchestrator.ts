 ...
 *   pnpm --filter @workspace/scripts run scan -- --agency=egged
 *   pnpm --filter @workspace/scripts run scan -- --agency busnearby
 *   pnpm --filter @workspace/scripts run scan -- --all
 *
 * npm:
 *   npm run scan --prefix scripts -- --agency=dan
 *   npm run scan --prefix scripts -- --all
 */
...
async function main() {
  ...
  if (!all && !agency) {
    ...
    console.error(`Example: npm run scan --prefix scripts -- --agency=egged`);
    process.exit(1);
  }
